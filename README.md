...
**双向私聊机器人**，并使用主题（Topics）管理私聊、配置自动回复 / 过滤、
自定义启动语与多媒体等功能。
...
- ✏️ **可自定义启动语**：支持文本、内联按钮与**图片 / 视频等多媒体**封面（Web 后台与 Bot 面板均可配置）。
- ⛔ 封禁 / 解封、用户搜索、拦截日志筛选、增强统计（今日/近 7 天活跃、拦截原因分布等）。
- 🖥 **Web 管理后台（Mini App）**：与 Bot 能力对齐——启动语媒体、自动回复媒体与「命中拦截」、按 ID 封禁、Topics 状态、群发进度回执、过滤词/自动回复导入导出；向机器人私聊发送媒体可获取 `file_id` 便于粘贴配置。
- 👥 **用户运营**：备注 / 标签 / 会话状态（未处理·处理中·已解决）、快捷回复模板、离开自动回复。
- 🧑‍💼 **多成员协作**：`owner` / `admin` / `support` 角色（Web 与 Bot 统一鉴权）；操作审计日志。
- 🏥 **平台健康**：超级管理员可在平台设置查看租户健康；Web 提供 `/api/platform/*`（平台 Token + 超管 initData）。
- 🎛️ **一站式控制面板**：拥有者 `/panel`（或 `/start`）即可直达自动回复、启动语、群发、安全过滤与 Topics 协作等常用功能，全程点按钮、无需记忆指令；用户 `/start` 自动显示自定义导航按钮，少打字、好上手。Web 启用后作为主控台，`/panel` 仍作轻量快捷入口。
- 📨 **送达回执**：用户消息成功转发给管理员后，会收到「✅ 已发送成功」轻提示，并在 5 秒后自动删除，保持对话整洁（可用 `messages.received` 自定义为持久回执）。
...
> 💡 平台主机器人与每个租户机器人**不再使用左下角「/」命令栏**：启动时会主动清空命令菜单，
> 全程改用**内联按钮面板**交互（平台 `/start` 的「🪄 创建我的机器人 / 我的机器人 / 常见问题」，
> 租户拥有者的 `/panel` 控制面板，用户 `/start` 的功能导航按钮）。命令本身仍可手动输入，
> 但不再展示在输入框旁，避免界面杂乱、降低记忆负担。
...
> 用按钮直达自动回复、启动语、群发、强制订阅，并一键开关防刷屏 / 英文拦截、管理
> 封禁用户、查看 Topics 状态与指令速查，无需记忆命令。
...
| `/panel` | 打开一站式控制面板（自动回复 / 启动语 / 群发 / 一键开关 / 封禁管理 / Topics 状态 / 指令速查） |
...
| `/filter_add <关键词>` | 添加过滤词（命中则拦截）；也可用控制面板或 Web 管理后台添加 / 编辑 / 删除 |
| `/filter_list`、`/filter_del <id>` | 列出 / 删除过滤词 |
| `/antiflood on｜off` | 防刷屏过滤器开关（默认开启） |
| `/alphabet_latin on｜off` | 屏蔽含拉丁字母（英文）的消息（默认关闭） |

> 💡 自动回复也可在 **⚙️ 控制面板 → 💬 自动回复** 中按钮式新增 / **编辑** / 删除，
> 并可选择**包含 / 正则**匹配、附带内联按钮与**图片 / 视频等多媒体**。
> 过滤词同样可在 **控制面板 → 🚫 过滤词** 或 **Web 管理后台 → 过滤词** 中查看列表、
> **编辑**与删除（支持分页与搜索）。
> 按钮支持每行一个或多个：用 `&&` 即可在同一行放置多个按钮
> （`文字 - 链接 && 文字 - 链接`），启动语按钮亦同。

### 3. 租户机器人 —— 普通用户指令

> 💡 用户发送 `/start` 后，会显示机器人拥有者配置的自定义启动语
> （可含图片 / 视频封面与导航按钮），点击按钮即可，无需记忆命令。

| 指令 | 说明 |
| --- | --- |
| `/start` | 开始私聊（消息转发给管理员），并显示自定义导航按钮 |
